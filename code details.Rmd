...
To start with, `p_formula` is a one-sided formula that specifies how  null-space of the principal splines is constructed. As in the `lm()` function, the intercept is always included unless otherwise stated. The basis matrix is `B = cbind(U, F)`, where the vectors in `U` span the nullspace, and `F` is the matrix of eigenvectors corresponding to positive eigenvalues of the bending energy matrix. The following table specifies how `p_formula` and `options_model` are related. Also, `~coords` is a shortcut used here to indicate `~lngcoords+latcoords`, i.e. the names of the 2 columns of the `coords` dataframe object. `q` represents the dimension of the nullspace and is such that `ncol(F) = n0-q`, where `n0` is the number of sampled locations.
